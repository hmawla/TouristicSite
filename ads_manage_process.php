...
$sql_b = "UPDATE ads SET ads_state = '1' WHERE id = $block";
if (mysqli_query($conn, $sql_b)) {
	header('location: user.php');
} else {
    $R = "Error renewed ads " . mysqli_error($conn);
}
$_SESSION['block'] = $R;
}
if(isset($_POST['s']))
...
$sql_un = "UPDATE ads SET ads_state = '0' WHERE id = $unblock";
if (mysqli_query($conn, $sql_un)) {
	header('location: user.php');

} else {
	header("location: user_manage.php");
    $R = "Error stoping ads " . mysqli_error($conn);
}
$_SESSION['unblock'] = $R;
}*/
?>